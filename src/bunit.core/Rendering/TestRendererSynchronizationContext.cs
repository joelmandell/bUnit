...
internal class TestRendererSynchronizationContext //: SynchronizationContext
{
	...
	private readonly SynchronizationContext testFrameworkSyncContext;

	public event UnhandledExceptionEventHandler UnhandledException;

	public TestRendererSynchronizationContext(SynchronizationContext testFrameworkSyncContext)
		: this(new State(), testFrameworkSyncContext)
	{
	}

	private TestRendererSynchronizationContext(State state, SynchronizationContext testFrameworkSyncContext)
	{
		this.state = state;
		this.testFrameworkSyncContext = testFrameworkSyncContext;
	}
	...
	public override void Post(SendOrPostCallback d, object state)
	{
		...
		{
			this.state.Task = Enqueue(this.state.Task, d, state, forceAsync: true);
		}
	}
	...
	public override void Send(SendOrPostCallback d, object state)
	{
		...
		lock (this.state.Lock)
		{
			antecedent = this.state.Task;
			this.state.Task = completion.Task;
		}
	...
	{
		return new TestRendererSynchronizationContext(state, testFrameworkSyncContext);
	}
	...
	private void ExecuteSynchronouslyIfPossible(SendOrPostCallback d, object state)
	{
		...
		lock (this.state.Lock)
		{
			...
			{
				this.state.Task = Enqueue(this.state.Task, d, state);
				return;
			}
	...
	{
		public TestRendererSynchronizationContext SynchronizationContext;
